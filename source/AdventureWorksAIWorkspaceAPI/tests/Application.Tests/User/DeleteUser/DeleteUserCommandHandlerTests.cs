...
public sealed class DeleteUserCommandHandlerTests
{
    ...
    public async Task Handle_WhenSuccess_ShouldDeleteUser()
    {
        ...
        await DeleteUserCommandHandler.Handle(command, _userService, CancellationToken.None);

        _ = _userService.Received(1).DeleteUserAsync("target-id", Arg.Any<CancellationToken>());
    }
    ...
    public async Task Handle_WhenCurrentUserIdIsMissing_ShouldThrowUnauthorizedException(string? currentUserId)
    {
        ...
        var act = () => DeleteUserCommandHandler.Handle(command, _userService, CancellationToken.None);

        await act.Should().ThrowAsync<UnauthorizedException>();
        _ = _userService.DidNotReceive().DeleteUserAsync(Arg.Any<string>(), Arg.Any<CancellationToken>());
    }
